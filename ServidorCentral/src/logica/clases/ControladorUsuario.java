...
public class ControladorUsuario implements IUsuario {
	...
	public void seguirONoAUsuario(String nickNameUsuario, String nickNameASeguir) {
		Manejador manejador = Manejador.getInstancia();
		Usuario usuario = manejador.getUsuarios().get(nickNameUsuario);
		Usuario aSeguir = manejador.getUsuarios().get(nickNameASeguir);
		if (usuario.getSeguidos().containsKey(nickNameASeguir)) { //ocurre cuando ya lo sigue
			usuario.getSeguidos().remove(nickNameASeguir);
			aSeguir.getSeguidores().remove(nickNameUsuario);
		} else { //ocurre cuando no lo sigue
			usuario.getSeguidos().put(nickNameASeguir, aSeguir);
			aSeguir.getSeguidores().put(nickNameUsuario, usuario);
		}
	}
